...
open_app_tool = {
    "name": "open_app",
    "description": "Opens an installed desktop application or supported web app on Windows, such as VS Code, Chrome, Spotify, WhatsApp, Discord, Telegram, Notepad, Calculator, File Explorer, Terminal, Office apps, or Settings.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "app_name": {
                "type": "STRING",
                "description": "The app name to open, for example: vscode, chrome, spotify, whatsapp, discord, telegram, word, excel, powerpoint, notepad, calculator, file explorer, terminal, settings."
            }
        },
        "required": ["app_name"]
    }
...
open_website_tool = {
    ...
    "parameters": {
        ...
        "properties": {
            "url": {
                "type": "STRING",
                "description": "The website URL or domain to open, for example youtube.com or https://github.com."
            }
        },
        "required": ["url"]
    }
...
take_screenshot_tool = {
    ...
    "description": "Takes a screenshot of the main display and saves it inside the current Melvis project.",
    "parameters": {
        "type": "OBJECT",
        "properties": {}
    }
...
set_volume_tool = {
    "name": "set_volume",
    "description": "Controls the Windows system volume using volume up, volume down, mute, or unmute/toggle mute actions.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "action": {
                "type": "STRING",
                "description": "One of: up, down, mute, unmute. Unmute toggles mute because Windows media keys expose mute as a toggle."
            },
            "steps": {
                "type": "INTEGER",
                "description": "How many keypress steps to apply for up/down. Use 5 if the user does not specify."
            }
        },
        "required": ["action"]
    }
...
system_power_tool = {
    "name": "system_power",
    "description": "Performs safe Windows power/session actions: lock, sleep, restart, or shutdown. Restart and shutdown should always require explicit user confirmation.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "action": {
                "type": "STRING",
                "description": "One of: lock, sleep, restart, shutdown."
            }
        },
        "required": ["action"]
    }
...
tools = [{'google_search': {}}, {"function_declarations": [generate_cad, run_web_agent, create_project_tool, switch_project_tool, list_projects_tool, list_smart_devices_tool, control_light_tool, discover_printers_tool, print_stl_tool, get_print_status_tool, iterate_cad_tool, open_app_tool, open_website_tool, take_screenshot_tool, set_volume_tool, system_power_tool] + tools_list[0]['function_declarations'][1:]}]
...
class AudioLoop:
    def __init__(self, video_mode=DEFAULT_MODE, on_audio_data=None, on_video_frame=None, on_cad_data=None, on_web_data=None, on_transcription=None, on_tool_confirmation=None, on_cad_status=None, on_cad_thought=None, on_project_update=None, on_device_update=None, on_error=None, input_device_index=None, input_device_name=None, output_device_index=None, kasa_agent=None):
        ...
        # If melvis.py is in backend/, project root is one up
    ...
    def update_permissions(self, new_perms):
        print(f"[MELVIs DEBUG] [CONFIG] Updating tool permissions: {new_perms}")
    ...
    def resolve_tool_confirmation(self, request_id, confirmed):
        print(f"[MELVIS DEBUG] [RESOLVE] resolve_tool_confirmation called. ID: {request_id}, Confirmed: {confirmed}")
        if request_id in self._pending_confirmations:
            future = self._pending_confirmations[request_id]
            if not future.done():
                print(f"[MELVIS DEBUG] [RESOLVE] Future found and pending. Setting result to: {confirmed}")
                future.set_result(confirmed)
            else:
                 print(f"[MELVIS DEBUG] [WARN] Request {request_id} future already done. Result: {future.result()}")
        else:
            print(f"[MELVIS DEBUG] [WARN] Confirmation Request {request_id} not found in pending dict. Keys: {list(self._pending_confirmations.keys())}")
    ...
    def clear_audio_queue(self):
        ...
        try:
            ...
            while not self.audio_in_queue.empty():
                ...
                count += 1
            if count > 0:
                print(f"[MELVIS DEBUG] [AUDIO] Cleared {count} chunks from playback queue due to interruption.")
        except Exception as e:
            print(f"[MELVIS DEBUG] [ERR] Failed to clear audio queue: {e}")
    ...
    async def listen_audio(self):
        ...
        if self.input_device_name:
            print(f"[MELVIS] Attempting to find input device matching: '{self.input_device_name}'")
            ...
            if resolved_input_device_index is not None:
                print(f"[MELVIS] Resolved input device '{self.input_device_name}' to index {resolved_input_device_index} ({best_match})")
            else:
                print(f"[MELVIS] Could not find device matching '{self.input_device_name}'. Checking index...")
        ...
        if resolved_input_device_index is None and self.input_device_index is not None:
             try:
                 ...
                 print(f"[MELVIS] Requesting Input Device Index: {resolved_input_device_index}")
             except ValueError:
                 print(f"[MELVIS] Invalid device index '{self.input_device_index}', reverting to default.")
                 resolved_input_device_index = None

        if resolved_input_device_index is None:
             print("[MELVIS] Using Default Input Device")
        ...
        try:
            ...
            print(f"[MELVIS] [ERR] Failed to open audio input stream: {e}")
            print("[MELVIS] [WARN] Audio features will be disabled. Please check microphone permissions.")
            return
            ...
            try:
                ...
                if rms > VAD_THRESHOLD:
                    ...
                    if not self._is_speaking:
                        ...
                        print(f"[MELVIS DEBUG] [VAD] Speech Detected (RMS: {rms}). Sending Video Frame.")
                        
                        # Send ONE frame
                        if self._latest_image_payload and self.out_queue:
                            await self.out_queue.put(self._latest_image_payload)
                        else:
                            print(f"[MELVIS DEBUG] [VAD] No video frame available to send.")
                    ...
                    if self._is_speaking:
                        if self._silence_start_time is None:
                            ...
                            print(f"[MELVIS DEBUG] [VAD] Silence detected. Resetting speech state.")
                            self._is_speaking = False
                            self._silence_start_time = None
    ...
    async def handle_cad_request(self, prompt):
        print(f"[MELVIS DEBUG] [CAD] Background Task Started: handle_cad_request('{prompt}')")
        if self.on_cad_status:
        ...
        if self.project_manager.current_project == "temp":
            ...
            print(f"[MELVIS DEBUG] [CAD] Auto-creating project: {new_project_name}")
            ...
            if success:
                ...
                try:
                    ...
                    print(f"[MELVIs DEBUG] [ERR] Failed to notify auto-project: {e}")
        ...
        if cad_data:
            print(f"[MELVIs DEBUG] [OK] CadAgent returned data successfully.")
            print(f"[MELVIs DEBUG] [INFO] Data Check: {len(cad_data.get('vertices', []))} vertices, {len(cad_data.get('edges', []))} edges.")
            
            if self.on_cad_data:
                print(f"[MELVIS DEBUG] [SEND] Dispatching data to frontend callback...")
                self.on_cad_data(cad_data)
                print(f"[MELVIS DEBUG] [SENT] Dispatch complete.")
            ...
            try:
                ...
                print(f"[MELVIS DEBUG] [NOTE] Sent completion notification to model.")
            except Exception as e:
                 print(f"[MELVIS DEBUG] [ERR] Failed to send completion notification: {e}")

        else:
            print(f"[MELVIS DEBUG] [ERR] CadAgent returned None.")
    ...
    async def handle_write_file(self, path, content):
        print(f"[MELVIS DEBUG] [FS] Writing file: '{path}'")
        ...
        if self.project_manager.current_project == "temp":
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            new_project_name = f"Project_{timestamp}"
            print(f"[MELVIS DEBUG] [FS] Auto-creating project: {new_project_name}")
        ...
        print(f"[MELVIS DEBUG] [FS] Resolved path: '{final_path}'")
        ...
        print(f"[MELVIS DEBUG] [FS] Result: {result}")
        try:
             await self.session.send(input=f"System Notification: {result}", end_of_turn=True)
        except Exception as e:
             print(f"[MELVIS DEBUG] [ERR] Failed to send fs result: {e}")

    async def handle_read_directory(self, path):
        print(f"[MELVIS DEBUG] [FS] Reading directory: '{path}'")
        try:
    ...
    async def handle_read_file(self, path):
        print(f"[MELVIS DEBUG] [FS] Reading file: '{path}'")
        try:
            if not os.path.exists(path):
    ...
    async def handle_open_app(self, app_name):
        """
        Open common Windows apps without confirmation when permissions set them to False.
        Uses aliases + multiple fallbacks so apps like Spotify/WhatsApp work whether they
        are installed as desktop apps, Microsoft Store apps, URI apps, or web apps.
        """
        import subprocess
        import webbrowser
        ...
        if not normalized:
            result = "No app name was provided."
            print(f"[MELVIS DEBUG] [APP] {result}")
            try:
                await self.session.send(input=f"System Notification: {result}", end_of_turn=True)
            except Exception as e:
                print(f"[MELVIS DEBUG] [ERR] Failed to send app result: {e}")
            return

        # Each app can have multiple fallback commands.
        # Melvis tries them in order until one launches without throwing an error.
        apps = {
            # Browsers
            "chrome": ["chrome.exe"],
            ...
            "whatsapp web": ["cmd /c start "" https://web.whatsapp.com"],
            "telegram": [
                "cmd /c start "" tg:",
                "Telegram.exe",
                "cmd /c start "" https://web.telegram.org"
            ],
            "discord": ["Discord.exe", "cmd /c start "" https://discord.com/app"],
            "slack": ["slack.exe", "cmd /c start "" https://app.slack.com"],
            "teams": ["msteams.exe", "Teams.exe", "cmd /c start "" msteams:"],
            "microsoft teams": ["msteams.exe", "Teams.exe", "cmd /c start "" msteams:"],
            "zoom": ["Zoom.exe", "cmd /c start "" zoommtg:"],
            "skype": ["Skype.exe", "cmd /c start "" skype:"],

            # Music / media
            "spotify": [
                "cmd /c start "" spotify:",
                "Spotify.exe",
                "cmd /c start "" https://open.spotify.com"
            ],
            "youtube": ["cmd /c start "" https://youtube.com"],
            "netflix": ["cmd /c start "" https://netflix.com"],
            ...
            # Design / creativity
            "figma": ["Figma.exe", "cmd /c start "" https://figma.com/files"],
            "canva": ["cmd /c start "" https://canva.com"],
            "blender": ["blender.exe"],
            "photoshop": ["Photoshop.exe"],
            "illustrator": ["Illustrator.exe"],
        }

        commands = apps.get(normalized, [app_name])
        ...
        if result is None:
            result = f"Failed to open {app_name}: {str(last_error)}"
    ...
    async def handle_open_website(self, url):
        """Open a website in the default browser."""
        import webbrowser

        clean_url = (url or "").strip()
        if not clean_url:
            result = "No website URL was provided."
        else:
            if not clean_url.startswith(("http://", "https://")):
                clean_url = "https://" + clean_url
            try:
                webbrowser.open(clean_url)
                result = f"Opened {clean_url}."
            except Exception as e:
                result = f"Failed to open {clean_url}: {str(e)}"

        print(f"[MELVIS DEBUG] [WEB-OPEN] {result}")
        try:
            await self.session.send(input=f"System Notification: {result}", end_of_turn=True)
        except Exception as e:
            print(f"[MELVIS DEBUG] [ERR] Failed to send website result: {e}")

    async def handle_take_screenshot(self):
        """Take a screenshot and save it in the active project's screenshots folder."""
        import mss.tools
        from datetime import datetime

        try:
            screenshots_dir = self.project_manager.get_current_project_path() / "screenshots"
        ...
        print(f"[MELVIS DEBUG] [SCREENSHOT] {result}")
        try:
            await self.session.send(input=f"System Notification: {result}", end_of_turn=True)
        except Exception as e:
            print(f"[MELVIS DEBUG] [ERR] Failed to send screenshot result: {e}")

    async def handle_set_volume(self, action, steps=5):
        """Control Windows volume using media key events. This avoids extra dependencies."""
        import ctypes
        import time
        ...
        try:
            if sys.platform != "win32":
                result = "Volume control is currently implemented for Windows only."
            elif action == "up":
                for _ in range(steps):
                    press_key(VK_VOLUME_UP)
                result = f"Volume increased by {steps} steps."
            elif action == "down":
                for _ in range(steps):
                    press_key(VK_VOLUME_DOWN)
                result = f"Volume decreased by {steps} steps."
            elif action in ["mute", "unmute", "toggle_mute"]:
                press_key(VK_VOLUME_MUTE)
                result = "Volume mute toggled."
            else:
                result = "Unknown volume action. Use up, down, mute, or unmute."
        except Exception as e:
            result = f"Failed to control volume: {str(e)}"

        print(f"[MELVIS DEBUG] [VOLUME] {result}")
        try:
            await self.session.send(input=f"System Notification: {result}", end_of_turn=True)
        except Exception as e:
            print(f"[MELVIS DEBUG] [ERR] Failed to send volume result: {e}")

    async def handle_system_power(self, action):
        """Run Windows session/power actions after the existing confirmation layer approves them."""
        import subprocess
        ...
        try:
            if sys.platform != "win32":
                result = "System power controls are currently implemented for Windows only."
            elif action == "lock":
                subprocess.Popen("rundll32.exe user32.dll,LockWorkStation", shell=True)
                result = "Locked the computer."
            elif action == "sleep":
                subprocess.Popen("rundll32.exe powrprof.dll,SetSuspendState 0,1,0", shell=True)
                result = "Sleep command sent."
            elif action == "restart":
                subprocess.Popen("shutdown /r /t 5", shell=True)
                result = "Restart scheduled in 5 seconds."
            elif action == "shutdown":
                subprocess.Popen("shutdown /s /t 5", shell=True)
                result = "Shutdown scheduled in 5 seconds."
            else:
                result = "Unknown system action. Use lock, sleep, restart, or shutdown."
        except Exception as e:
            result = f"Failed to perform system action '{action}': {str(e)}"

        print(f"[MELVIS DEBUG] [POWER] {result}")
        try:
            await self.session.send(input=f"System Notification: {result}", end_of_turn=True)
        except Exception as e:
            print(f"[MELVIS DEBUG] [ERR] Failed to send power result: {e}")

    async def handle_web_agent_request(self, prompt):
        print(f"[MELVIS DEBUG] [WEB] Web Agent Task: '{prompt}'")
        ...
        print(f"[MELVIS DEBUG] [WEB] Web Agent Task Returned: {result}")
        ...
        try:
             ...
             print(f"[MELVIS DEBUG] [ERR] Failed to send web agent result to model: {e}")
                ...
                async for response in turn:
                            ...
                            if transcript:
                                ...
                                if transcript != self._last_output_transcription:
                                    ...
                                    if delta:
                                        # Send to frontend (Streaming)
                                        if self.on_transcription:
                                             self.on_transcription({"sender": "MELVIS", "text": delta})
                                        
                                        # Buffer for Logging
                                        if self.chat_buffer["sender"] != "MELVIS":
                                            # Flush previous
                                            if self.chat_buffer["sender"] and self.chat_buffer["text"].strip():
                                                self.project_manager.log_chat(self.chat_buffer["sender"], self.chat_buffer["text"])
                                            # Start new
                                            self.chat_buffer = {"sender": "MELVIS", "text": delta}
                                        else:
                                            # Append
                                            self.chat_buffer["text"] += delta
                    ...
                    if response.tool_call:
                        ...
                        for fc in response.tool_call.function_calls:
                            if fc.name in ["generate_cad", "run_web_agent", "write_file", "read_directory", "read_file", "create_project", "switch_project", "list_projects", "list_smart_devices", "control_light", "discover_printers", "print_stl", "get_print_status", "iterate_cad", "open_app", "open_website", "take_screenshot", "set_volume", "system_power"]:
                                prompt = fc.args.get("prompt", "") # Prompt is not present for all tools
                                
                                # Check Permissions
                                # IMPORTANT:
                                #   False = run immediately without asking
                                #   True  = ask frontend for confirmation first
                                confirmation_required = self.permissions.get(fc.name, True)

                                print(f"[MELVIS DEBUG] [PERMISSIONS] Tool='{fc.name}' confirmation_required={confirmation_required}")
                                print(f"[MELVIS DEBUG] [PERMISSIONS] Current permissions={self.permissions}")

                                if confirmation_required:
                                    if not self.on_tool_confirmation:
                                        print(f"[MELVIS DEBUG] [WARN] Confirmation required for '{fc.name}', but no confirmation callback exists.")
                                        function_response = types.FunctionResponse(
                                            id=fc.id,
                                            name=fc.name,
                                            response={
                                                "result": "Confirmation is required, but the frontend confirmation system is unavailable.",
                                            }
                                        )
                                    ...
                                    print(f"[MELVIS DEBUG] [STOP] Requesting confirmation for '{fc.name}' (ID: {request_id})")
                                    ...
                                    print(f"[MELVIS DEBUG] [CONFIRM] Request {request_id} resolved. Confirmed: {confirmed}")

                                    if not confirmed:
                                        print(f"[MELVIS DEBUG] [DENY] Tool call '{fc.name}' denied by user.")
                                        function_response = types.FunctionResponse(
                                            id=fc.id,
                                            name=fc.name,
                                            response={
                                    ...
                                    print(f"[MELVIS DEBUG] [AUTO-ALLOW] Running '{fc.name}' without confirmation.")
                                # If confirmed (or no callback configured, or auto-allowed), proceed
                                if fc.name == "generate_cad":
                                    print(f"\n[MELVIS DEBUG] --------------------------------------------------")
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call Detected: 'generate_cad'")
                                    print(f"[MELVIS DEBUG] [IN] Arguments: prompt='{prompt}'")
                                    ...
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'run_web_agent' with prompt='{prompt}'")
                                    ...
                                    print(f"[MELVIS DEBUG] [RESPONSE] Sending function response: {function_response}")
                                    function_responses.append(function_response)
                                    ...
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'open_app' app_name='{app_name}'")
                                    ...
                                    url = fc.args["url"]
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'open_website' url='{url}'")
                                    asyncio.create_task(self.handle_open_website(url))
                                    function_response = types.FunctionResponse(
                                    ...
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'take_screenshot'")
                                    asyncio.create_task(self.handle_take_screenshot())
                                    function_response = types.FunctionResponse(
                                        id=fc.id, name=fc.name, response={"result": "Taking screenshot..."}
                                    )
                                    function_responses.append(function_response)

                                elif fc.name == "set_volume":
                                    action = fc.args["action"]
                                    steps = fc.args.get("steps", 5)
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'set_volume' action='{action}' steps='{steps}'")
                                    asyncio.create_task(self.handle_set_volume(action, steps))
                                    function_response = types.FunctionResponse(
                                        id=fc.id, name=fc.name, response={"result": f"Volume action '{action}' started."}
                                    )
                                    function_responses.append(function_response)

                                elif fc.name == "system_power":
                                    action = fc.args["action"]
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'system_power' action='{action}'")
                                    asyncio.create_task(self.handle_system_power(action))
                                    function_response = types.FunctionResponse(
                                        id=fc.id, name=fc.name, response={"result": f"System action '{action}' started."}
                                    )
                                    function_responses.append(function_response)
                                    ...
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'write_file' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'read_directory' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'read_file' path='{path}'")
                                    ...
                                    name = fc.args["name"]
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'create_project' name='{name}'")
                                    ...
                                    name = fc.args["name"]
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'switch_project' name='{name}'")
                                    ...
                                    if success:
                                        ...
                                        print(f"[MELVIS DEBUG] [PROJECT] Sending project context to AI ({len(context)} chars)")
                                        try:
                                            await self.session.send(input=f"System Notification: {msg}\n\n{context}", end_of_turn=False)
                                        except Exception as e:
                                            print(f"[MELVIS DEBUG] [ERR] Failed to send project context: {e}")
                                    function_response = types.FunctionResponse(
                                        id=fc.id, name=fc.name, response={"result": msg}
                                    )
                                    function_responses.append(function_response)
                                    ...
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'list_projects'")
                                    ...
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'list_smart_devices'")
                                    ...
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'control_light' Target='{target}' Action='{action}'")
                                    ...
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'discover_printers'")
                                    ...
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'print_stl' STL='{stl_path}' Printer='{printer}'")
                                    ...
                                    printer = fc.args["printer"]
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'get_print_status' Printer='{printer}'")
                                    ...
                                    print(f"[MELVIS DEBUG] [TOOL] Tool Call: 'iterate_cad' Prompt='{prompt}'")
                                    ...
                                    if cad_data:
                                        print(f"[MELVIS DEBUG] [OK] CadAgent iteration returned data successfully.")
                                        
                                        # Dispatch to frontend
                                        if self.on_cad_data:
                                            print(f"[MELVIS DEBUG] [SEND] Dispatching iterated CAD data to frontend...")
                                            self.on_cad_data(cad_data)
                                            print(f"[MELVIS DEBUG] [SENT] Dispatch complete.")
                                        ...
                                        print(f"[MELVIS DEBUG] [ERR] CadAgent iteration returned None.")
    ...
    async def run(self, start_message=None):
        ...
        while not self.stop_event.is_set():
            try:
                print(f"[MELVIS DEBUG] [CONNECT] Connecting to Gemini Live API...")
                async with (
                    ...
                    if not is_reconnect:
                        if start_message:
                            print(f"[MELVIS DEBUG] [INFO] Sending start message: {start_message}")
                        ...
                        print(f"[MELVIS DEBUG] [RECONNECT] Connection restored.")
                        # Restore Context
                        print(f"[MELVIS DEBUG] [RECONNECT] Fetching recent chat history to restore context...")
                        ...
                        print(f"[MELVIS DEBUG] [RECONNECT] Sending restoration context to model...")
                ...
                print(f"[MELVIS DEBUG] [STOP] Main loop cancelled.")
                break
                
            except Exception as e:
                # This catches the ExceptionGroup from TaskGroup or direct exceptions
                print(f"[MELVIS DEBUG] [ERR] Connection Error: {e}")
                
                if self.stop_event.is_set():
                    break
                
                print(f"[MELVIS DEBUG] [RETRY] Reconnecting in {retry_delay} seconds...")
