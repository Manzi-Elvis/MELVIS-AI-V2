...
@app.get("/status")
async def status():
    return {"status": "running", "service": "M.E.L.V.I.S Backend"}

@sio.event
async def connect(sid, environ):
    print(f"Client connected: {sid}")
    await sio.emit('status', {'msg': 'Connected to M.E.L.V.I.S Backend'}, room=sid)
...
async def start_audio(sid, data=None):
    global audio_loop, loop_task, printer_monitor_task
    ...
    if audio_loop:
        if loop_task and (loop_task.done() or loop_task.cancelled()):
             ...
             await sio.emit('status', {'msg': 'M.E.L.V.I.S Already Running'})
             return
    ...
    def on_cad_status(status):
        # status can be: 
        # - a string like "generating" (from melvis.py handle_cad_request)
    ...
    try:
        ...
        print("Emitting 'M.E.L.V.I.S Started'")
        await sio.emit('status', {'msg': 'M.E.L.V.I.S Started'})
        ...
        # Start Printer Monitor only once
        if printer_monitor_task is None or printer_monitor_task.done():
            printer_monitor_task = asyncio.create_task(monitor_printers_loop())
        
    except Exception as e:
        print(f"CRITICAL ERROR STARTING MELVIS: {e}")
        import traceback
        traceback.print_exc()
...
async def stop_audio(sid):
    ...
    await sio.emit('status', {'msg': 'M.E.L.V.I.S Stopped'})
...
async def shutdown(sid, data=None):
    ...
    global audio_loop, loop_task, printer_monitor_task, authenticator
    ...
    # Cancel printer monitor if running
    if printer_monitor_task and not printer_monitor_task.done():
        print("[SERVER] Cancelling printer monitor task...")
        printer_monitor_task.cancel()
        printer_monitor_task = None
...
async def user_input(sid, data):
    ...
    if not audio_loop:
        ...
        await sio.emit('error', {'msg': 'M.E.L.V.I.S is not running. Start audio first.'}, room=sid)
        return

    if not audio_loop.session:
        print("[SERVER DEBUG] [Error] Session is None. Cannot send text.")
        await sio.emit('error', {'msg': 'M.E.L.V.I.S session is not ready yet.'}, room=sid)
        return
...
async def save_memory(sid, data):
    try:
        ...
        await sio.emit('memory_saved', {'filename': str(filename)})
...
async def discover_printers(sid):
    ...
    if not audio_loop or not audio_loop.printer_agent:
        ...
        if saved_printers:
            ...
            await sio.emit('status', {'msg': "Connect to M.E.L.V.I.S to enable printer discovery"})
            return
...
@sio.event
async def open_app(sid, data):
    """Direct frontend shortcut for opening a desktop app through MELVIS."""
    app_name = data.get('app_name') or data.get('name')
    if not app_name:
        await sio.emit('error', {'msg': 'No app name provided'}, room=sid)
        return
    if not audio_loop:
        await sio.emit('error', {'msg': 'M.E.L.V.I.S is not running. Start audio first.'}, room=sid)
        return
    asyncio.create_task(audio_loop.handle_open_app(app_name))
    await sio.emit('status', {'msg': f'Opening {app_name}...'})

@sio.event
async def open_website(sid, data):
    """Direct frontend shortcut for opening a website through MELVIS."""
    url = data.get('url')
    if not url:
        await sio.emit('error', {'msg': 'No website URL provided'}, room=sid)
        return
    if not audio_loop:
        await sio.emit('error', {'msg': 'M.E.L.V.I.S is not running. Start audio first.'}, room=sid)
        return
    asyncio.create_task(audio_loop.handle_open_website(url))
    await sio.emit('status', {'msg': f'Opening {url}...'})

@sio.event
async def take_screenshot(sid, data=None):
    """Direct frontend shortcut for taking a screenshot through MELVIS."""
    if not audio_loop:
        await sio.emit('error', {'msg': 'M.E.L.V.I.S is not running. Start audio first.'}, room=sid)
        return
    asyncio.create_task(audio_loop.handle_take_screenshot())
    await sio.emit('status', {'msg': 'Taking screenshot...'})

@sio.event
async def set_volume(sid, data):
    """Direct frontend shortcut for system volume control through MELVIS."""
    action = data.get('action')
    level = data.get('level')
    if not audio_loop:
        await sio.emit('error', {'msg': 'M.E.L.V.I.S is not running. Start audio first.'}, room=sid)
        return
    asyncio.create_task(audio_loop.handle_set_volume(action, level))
    await sio.emit('status', {'msg': 'Updating volume...'})
